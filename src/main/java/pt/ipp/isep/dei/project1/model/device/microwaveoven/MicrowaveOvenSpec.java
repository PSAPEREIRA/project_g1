...
public class MicrowaveOvenSpec implements DeviceSpecs {
    ...
    private final List<Double> mValueOfAttributesCaseMicrowaveOven = new ArrayList<>();

    public MicrowaveOvenSpec() {
        addAttributeNames();
        addAttributeValues();
    }

    public MicrowaveOvenSpec (double nominalPower) {
        addAttributeNames();
        mValueOfAttributesCaseMicrowaveOven.add(nominalPower);
    }
    ...
    public void addAttributeValues() {
        mValueOfAttributesCaseMicrowaveOven.add(0.0);
    }
    ...
    public double getAttributeValue(String attributeName) {
        ...
        for (index = 0; index < attributeNames.size(); index++) {
            if (attributeNames.get(index).equals(attributeName))
                return mValueOfAttributesCaseMicrowaveOven.get(index);
        }
        return Double.NaN;
    }
    ...
    public void setAttributeValue(String attributeName, double value) {
        ...
        for (index = 0; index < attributeNames.size(); index++) {
            if (attributeNames.get(index).equals(attributeName))
                mValueOfAttributesCaseMicrowaveOven.set(index, value);
        }
    }
    ...
    public List<String> getAttributeNamesAndValues() {
        ...
        listOfNamesAndValues.add(attributeNames.get(0) + " - " + mValueOfAttributesCaseMicrowaveOven.get(0));
